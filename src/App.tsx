...
function App() {
  ...
  function handleClick_shift(){
    ...
    indices.map((index)=>{
    new_indices.push((index+key_shift)%26)
    });
    new_indices.map((letter)=>{
      console.log(array[letter])
      new_word.push(array[letter])
  ...
  function handleClick_affine(){
    ...
    indices.map((index)=>{
    console.log(index,key_affine1,key_affine2,(key_affine1*index)+key_affine2)
...
const genRand = (len) => {
 ...
 function handleClick_vernam(){
  let binary_key=genRand(word_vernam.length+3)
  let binary_word=convert(word_vernam)
  console.log(convert( (binary_word^binary_key).toString() ))
  setChiper_vernam(convert( (binary_word^binary_key).toString() ))
